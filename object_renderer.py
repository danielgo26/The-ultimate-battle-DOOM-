...
class ObjectRenderer:
    ...
    def draw_player_health(self):
        ...
        for i, char in enumerate(health):
            ...
            image_rect = image.get_rect(topleft=(i * self.digit_size + 40, 30))
            self.screen.blit(image, image_rect)
        image = self.digits['10']
        image_rect = image.get_rect(topleft=((i+1) * self.digit_size + 40, 30))
        self.screen.blit(image, image_rect)
    ...
    def draw_player_score(self):
        ...
        for i, char in enumerate(score):
            image = self.digits[char]
            image_rect = image.get_rect(topleft=(i * self.digit_size + 40, 150))
            self.screen.blit(image, image_rect)
        image = pg.image.load('resources/xp.png')
        image_rect = image.get_rect(topleft=((i + 1) * self.digit_size + 40, 200))
